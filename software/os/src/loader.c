...
static uintptr_t elf_load(const char *filename) {
    ...
    Elf_Ehdr elf_h;
    size_t disk_offset = offset(filename);
    printf("%d\n", disk_offset);
    
    ramdisk_read((void*)&elf_h,disk_offset,sizeof(Elf_Ehdr));
    assert(*((int32_t*)elf_h.e_ident)==0x464C457F);
    assert(elf_h.e_machine==EM_RISCV);

    Elf_Phdr elf_ph;
    int j=0;
    while(j<elf_h.e_phnum){
        uint64_t offset = elf_h.e_phoff + j * elf_h.e_phentsize;
        ramdisk_read((void*)&elf_ph,offset+disk_offset,sizeof(Elf_Phdr));
