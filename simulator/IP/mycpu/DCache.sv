...
/* -------------- 1 request buffer : lock the read request addr -------------- */
    ...
    always_comb begin
        uncached = addr_pipe[31:28]==4'hA ? 1 : 0;
    end
...
/* -------------- 8 read data control: choose data from mem or return buffer -------------- */
    ...
    assign rdata_ret    = uncached? {{(BIT_NUM-32){1'b0}},ret_buf[BIT_NUM-1:BIT_NUM-32]} : ret_buf >> {addr_pipe[BYTE_OFFSET_WIDTH-1:2], 5'b0};
...
/* -------------- 14 memory settings -------------- */
    ...
    assign d_raddr  = uncached ? addr_pipe : {addr_pipe[31:BYTE_OFFSET_WIDTH], {BYTE_OFFSET_WIDTH{1'b0}}};
    assign d_rsize  = uncached? rsize_pipe : 3'h2;
    assign d_rlen   = uncached? 8'b0 : WORD_NUM - 1;
    assign d_waddr  = maddr_buf;
    assign d_wsize  = 3'h2;
    assign d_wlen   = uncached ? 8'b0 : WORD_NUM - 1;
    assign d_wdata  = wbuf[31:0];
    assign d_wstrb  = uncached? wstrb_pipe : 4'b1111;
...
/* -------------- 16 write fsm: for write back-------------- */
    ...
    logic [WORD_OFFSET_WIDTH:0] write_num = uncached?0: WORD_NUM - 1;
